...
class DatasetManager(BaseModel):
    ...
    def _detect_format(self, *, filepath: Path) -> str:
        ...
        with open(filepath, mode='r', encoding=self.encoding) as f:
            lines = [f.readline().strip() for _ in range(min(5, sum(1 for _ in f) + 1))]
            f.seek(0)
            reader = csv.reader(f)
            header = next(reader)
...
def load_trajectory_csv(
    ...
    Args:
        filepath: Path to CSV file
        min_confidence: Optional minimum confidence threshold for DLC data

    Returns:
        TrajectoryDataset with loaded trajectories
    """
    manager = DatasetManager(min_confidence=min_confidence)
...
def save_solver_result(
    ...
    raise NotImplementedError("Saving SolverResult is not implemented yet.")