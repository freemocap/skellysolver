"""Demo script for rigid body tracking pipeline with fixed reference geometry.

Generates synthetic data with a cube + asymmetric markers,
adds noise, and optimizes ONLY the poses (rotations and translations)
while keeping the reference geometry fixed at median distances.

Now includes automatic HTML viewer generation!
"""
...
def create_cube_topology() -> RigidBodyTopology:
    ...
    # Cube edges (12 edges) + connections to asymmetric markers
    rigid_edges = [
        # Bottom face
        (0, 1), (1, 2), (2, 3), (3, 0),
        # Top face
...
def run_synthetic_demo() -> None:
    ...
    logger.info("=" * 80)
    logger.info("SYNTHETIC RIGID BODY DEMO - FIXED REFERENCE GEOMETRY")
    logger.info("=" * 80)
    ...
    config = RigidBodyPipelineConfig(
        ...
        # Note: No rigidity_weight, soft_weight, or anchor_weight
        # since reference geometry is fixed
    )
    ...
    logger.info("\nRunning pipeline...")
    # The run() method now automatically generates the HTML viewer!
    pipeline.setup_problem_and_solve()

    # 7. Evaluate results
    if pipeline.solver_result is not None and pipeline.solver_result.success:
        logger.info("\n" + "=" * 80)
        logger.info("EVALUATION")
        logger.info("=" * 80)

        # Compute accuracy vs ground truth
        accuracy_metrics = pipeline.solver_result.compute_reconstruction_accuracy(
        ...
        pose_csv = output_dir / "poses.csv"
        pose_dataset = pipeline.solver_result.get_pose_trajectory()
        save_trajectory_csv(dataset=pose_dataset, filepath=pose_csv)
        logger.info(f"\nSaved pose trajectory: {pose_csv}")
        ...
        logger.info(f"  - raw_data.csv (raw data for viewer)")
        logger.info(f"  - optimized_data.csv (optimized results)")
        logger.info(f"  - poses.csv (rotation matrices and translations)")
        logger.info(f"  - topology.json (rigid body structure)")
        logger.info(f"  - rigid_body_viewer.html (interactive 3D visualization)")
        logger.info(f"\n  → Open rigid_body_viewer.html in your browser to view results!")
    else:
        logger.error("Pipeline failed to produce results!")
        if pipeline.solver_result is not None:
            logger.error(f"Solver status: {pipeline.solver_result.summary.termination_type}")
