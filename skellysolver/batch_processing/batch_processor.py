...
class BatchJobResult(ABaseModel):
...
class BatchProcessor(ABaseModel):
    ...
    def _create_pipeline(self, *, config: PipelineConfig) -> BasePipeline:
        ...
        if "RigidBody" in config_class_name:
            from ..solvers.mocap_solver import RigidBodyPipeline
            return RigidBodyPipeline(config=config)
        elif "EyeTracking" in config_class_name:
            from skellysolver.solvers.eye_solver.eye_pipeline import EyeTrackingPipeline
